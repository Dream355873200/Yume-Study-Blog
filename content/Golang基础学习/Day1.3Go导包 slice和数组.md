...
go语言有个特殊的机制：**包的init函数**
`init` 函数的定义非常简单，**没有参数，也没有返回值**。
```go
package main
...
```
go语言的初始化机制是：
**递归执行main函数导入的包以及每个包导入的包**，比如main函数导入lib1，lib1导入lib2。
是先执行lib1的init，然后lib2的init。

所以实际上**包的init函数是先于main的init函数和main函数主体的**。

**顺序基本上就是按导入包的顺序执行**，然后才轮到main的init和main函数
...
```
最后一个_Test也是私有的，访问类型**只根据第一个字母区分**。
...
首先需要理解的是，**Go语言的规则是一切都是值传递**，不像c++有引用类型，在Go中官方说是没有引用的。
...
slice:= []int{}
```
注意：虽然它们很像但**却是截然不同的两种数据类型**
数组**存储的是固定长度的连续空间的值**。

而slice**底层是一个结构体，存储的其实有资源指针，长度，容量。**
slice实际上有点像std::vector，它也有**双倍扩容机制**。
...
```
它是在原数据上进行操作的，不是值拷贝。
遍历数组一般使用slice进行遍历，由于复制的只是资源指针，**性能好一些**。
...
```
定义时，会**开辟一片3长度的空间**，然后初始化长度为默认值。
它们都有有如下方法
cap()返回容量，len()返回长度，copy复制（开辟新空间）

需要注意的是，slice的copy如果创建的接收方切片太小，数据就装不下了，Go 不会自动帮你扩容，而是**直接“截断”**。
比如说接收方只有3，而复制5，只会复制前三个数据进去。
